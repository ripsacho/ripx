...
import { apiGet, apiPut, apiPost, apiDelete, isStandaloneMode } from '../../services';
...
const INTEGRATIONS_CONFIG = [
  {
    ...
    iconClass: 'ga4',
    configHint: 'Get Measurement ID and API secret from GA4 Admin → Data Streams → Web stream → Measurement Protocol API secrets.',
  },
  {
    key: 'bigquery',
    title: 'BigQuery',
    Icon: DataTableIcon,
    iconClass: 'bigquery',
    configHint: 'Paste your GCP service account JSON key. Create tables using backend/docs/bigquery_schema.sql',
  },
];
...
function Settings() {
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      const data = response.data?.settings || response.data?.data?.settings;
      if (data) {
        setSettings({
  ...
  const handleSaveIntegrations = useCallback(async () => {
    ...
    try {
      await apiPut('/settings/integrations', {
        ...
        ga4ApiSecret: integrationConfig.ga4ApiSecret && integrationConfig.ga4ApiSecret !== '••••••••' && integrationConfig.ga4ApiSecret !== ''
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      outboundWebhookEvents: Array.isArray(settings.outboundWebhookEvents) && settings.outboundWebhookEvents.length > 0
  ...
  const formatPresetSegments = (p) => {
    ...
    const parts = [
      ...
      Array.isArray(seg.countries) && seg.countries.length > 0 && `Countries: ${seg.countries.join(', ')}`,
    ]
      .filter(Boolean)
      .join(' · ');
  ...
  return (
    ...
    >
        ...
        <BlockStack gap={CONTENT_GAP}>
          {loading ? (
            <div className={styles.settingsLoadingSkeleton}>
              <div className={styles.loadingBlock} style={{ height: 80, marginBottom: '1.5rem' }} />
            ...
            <div className={styles.settingsPanels}>
              {selectedTab === 0 && (
                ...
                >
                  <Card className={`${styles.settingsPanelCard} ${styles.installMain} ${styles.storefrontSnippetCard}`}>
                    <Box padding="500">
                      <BlockStack gap="400">
                        <div className={styles.snippetSectionHeader}>
                          ...
                          <div className={styles.snippetSectionHeaderContent}>
                            ...
                            <Text as="p" variant="bodySm" tone="subdued" className={styles.snippetSectionDesc}>
                          ...
                          <div className={styles.snippetSection}>
                            ...
                            <div className={styles.snippetSubsection}>
                              <div className={styles.snippetSubsectionHeader}>
                                <span className={styles.snippetSubsectionLabel}>Script URL</span>
                              </div>
                              <div className={`${styles.snippetBlock} ${styles.snippetBlockInline}`}>
                                <code className={styles.snippetUrl}>{installation.scriptUrl}</code>
                  ...
                  <Card className={`${styles.settingsPanelCard} ${styles.installSide}`}>
                    <Box padding="500">
                      <BlockStack gap={CONTENT_GAP}>
                        ...
                        {installation.instructions?.steps && installation.instructions.steps.length > 0 && (
                          <>
                            <Text variant="headingSm" as="h3">Setup steps</Text>
                            <ol className={styles.installSteps}>
                              {installation.instructions.steps.map((step, i) => (
                                <li key={i}>
                                  <Text as="span" variant="bodyMd">{step}</Text>
                                </li>
                              ))}
                            </ol>
                          </>
                        ...
                        {installation.instructions?.altMethod && (
                          <>
                            <Text variant="headingSm" as="h3">Alternative: {installation.instructions.altMethod}</Text>
                            <div className={`${styles.snippetBlock} ${styles.snippetBlockAlt}`}>
                              <pre className={styles.snippetPre}>
                                <code>{installation.instructions.altSnippet}</code>
                              </pre>
                              <Button
                                icon={ClipboardIcon}
                                onClick={() => handleCopy(installation.instructions.altSnippet, 'Snippet copied')}
                                variant="plain"
                                size="slim"
                              >
                                Copy
                        ...
                        {(!installation.instructions?.steps?.length && !installation.instructions?.altMethod) && (
              ...
              {selectedTab === 1 && (
                ...
                >
                            ...
                            {Object.entries(SETTINGS_PRESETS).map(([key, preset]) => (
                              ...
                              >
                                ...
                                <span className={styles.presetCardMeta}>
                                  {preset.minSampleSize} visitors · {Math.round(preset.confidenceLevel * 100)}% confidence
                                </span>
                              </button>
                            ))}
                          </div>
                          ...
                          <div className={styles.testConfigCustom}>
                            ...
                            <div className={styles.configFieldGroups}>
                              <div className={styles.configFieldGroup}>
                                <Text variant="bodySm" fontWeight="semibold" as="span" className={styles.configFieldLabel}>
                                  Minimum Sample Size
                                </Text>
                                ...
                                <div className={styles.configTextField}>
                                  <TextField
                                    ...
                                    onChange={(value) => {
                                      ...
                                      setSettings({
                                        ...settings,
                                        minSampleSize: Number.isFinite(num) ? Math.max(10, Math.min(10000, num)) : 100,
                                      });
                                    }}
                                ...
                                <div className={styles.configTextField}>
                                  <TextField
                                    ...
                                    onChange={(value) => {
                                      ...
                                      setSettings({
                                        ...settings,
                                        confidenceLevel: Number.isFinite(num) ? Math.max(0.8, Math.min(0.99, num)) : 0.95,
                                      });
                                    }}
                      ...
                      <BlockStack gap={CONTENT_GAP}>
                        <div className={styles.sectionHeader}>
                          ...
                          <div className={styles.sectionHeaderContent}>
                            ...
                            <Text as="p" variant="bodySm" tone="subdued">
                              Send events to your server when tests complete or reach significance.
                            </Text>
                          </div>
                        </div>
                        <div className={styles.panelCardBody}>
                  ...
                  <Card className={`${styles.settingsPanelCard} ${styles.settingsPanelCardFull} ${styles.quickLinksCard}`}>
                    <Box padding="400">
                      <BlockStack gap="300">
                        <div className={styles.sectionHeader}>
                          <div className={styles.sectionHeaderIcon}>
                          ...
                          <div className={styles.sectionHeaderContent}>
                            <Text variant="headingMd" as="h2">User Preferences</Text>
                            <Text as="p" variant="bodySm" tone="subdued">
                              Notifications, dashboard defaults, theme (custom schedule), and export format are in{' '}
                              <Link to={ROUTES.PROFILE} className={styles.setupWizardLink}>Profile</Link>
              ...
              {selectedTab === 2 && (
                ...
                >
                  <Card className={`${styles.settingsPanelCard} ${styles.settingsPanelCardFull} ${styles.integrationsHeaderCard}`}>
                    <Box padding="400">
                      <div className={styles.sectionHeaderWithAction}>
                        <div className={styles.sectionHeader}>
                          ...
                          <div className={styles.sectionHeaderContent}>
                            ...
                            <Text as="p" variant="bodySm" tone="subdued">
                              Connect GA4 and BigQuery to unify analytics and run advanced queries.
                            </Text>
                          </div>
                        </div>
                        <Button
                  ...
                  <div className={styles.integrationCardsRow}>
                    {INTEGRATIONS_CONFIG.map(({ key, title, Icon, iconClass, configHint }) => {
                      ...
                      return (
                        <Card key={key} className={`${styles.settingsPanelCard} ${styles.integrationCardWrapper} ${configured ? styles.integrationCardConnected : ''}`}>
                          <Box padding="400">
                            <BlockStack gap={CONTENT_GAP}>
                              <div className={styles.sectionHeader}>
                                <div className={`${styles.sectionHeaderIcon} ${styles.integrationIcon} ${styles[iconClass]}`}>
                                  <Icon />
                                </div>
                                <div className={`${styles.sectionHeaderContent} ${styles.integrationCardHeader}`}>
                                  <div className={styles.integrationCardTitleRow}>
                                    <Text variant="headingMd" as="h2" className={styles.integrationCardTitle}>
                                      {title}
                                    </Text>
                                    {!isLoading && (
                                      <Badge tone={configured ? 'success' : 'info'} className={styles.integrationCardBadge}>
                                        {key === 'ga4'
                                  ...
                                  <Text as="p" variant="bodySm" tone="subdued" className={styles.integrationCardHint}>
                              ...
                              <div className={styles.panelCardBody}>
                                {isLoading ? (
                                  <div className={styles.loadingBlock} style={{ height: 60, marginTop: '0.5rem' }} />
                                ) : (
                                  <>
                                    {key === 'ga4' && (
                                      <FormLayout>
                                        <TextField
                                          ...
                                          onChange={(v) => setIntegrationConfig((c) => ({ ...c, ga4MeasurementId: v }))}
                                        ...
                                        <TextField
                                          ...
                                          onChange={(v) => setIntegrationConfig((c) => ({ ...c, ga4ApiSecret: v }))}
                                          placeholder={integrationConfig.ga4ApiSecret === '••••••••' ? '••••••••' : 'Enter API secret'}
                                          autoComplete="off"
                                    ...
                                    {key === 'bigquery' && (
                                      <FormLayout>
                                        <TextField
                                          ...
                                          onChange={(v) => setIntegrationConfig((c) => ({ ...c, bigqueryProjectId: v }))}
                                        ...
                                        <TextField
                                          ...
                                          onChange={(v) => setIntegrationConfig((c) => ({ ...c, bigqueryDataset: v }))}
                                          placeholder="ripx_analytics"
                                          autoComplete="off"
                                        />
                                        <TextField
                                          label="Service Account JSON"
                                          value={integrationConfig.bigqueryCredentials === '[configured]' ? '' : integrationConfig.bigqueryCredentials}
                                          onChange={(v) => setIntegrationConfig((c) => ({ ...c, bigqueryCredentials: v }))}
                                          placeholder={integrationConfig.bigqueryCredentials === '[configured]' ? '[Already configured — leave blank to keep]' : 'Paste full JSON key'}
                                          multiline={4}
                                          autoComplete="off"
                      ...
                      <BlockStack gap={CONTENT_GAP}>
                        <div className={styles.sectionHeader}>
                          ...
                          <div className={styles.sectionHeaderContent}>
                            ...
                            <Text as="p" variant="bodySm" tone="subdued">
                              Choose how the app looks. Auto switches by time of day. For custom schedule, see{' '}
                              <Link to={`${ROUTES.PROFILE}?tab=preferences`} className={styles.setupWizardLink}>
                                Profile → Preferences
                              </Link>
                              .
                            </Text>
                        ...
                        <div className={styles.themePreviewGrid}>
                          {THEME_OPTIONS.map((opt) => (
                            ...
                            >
                              <div className={`${styles.themePreviewSwatch} ${styles[`themePreviewSwatch_${opt.preview}`]}`} />
                      ...
                      <BlockStack gap={CONTENT_GAP}>
                        <div className={styles.sectionHeader}>
                          ...
                          <div className={styles.sectionHeaderContent}>
                            ...
                            <Text as="p" variant="bodySm" tone="subdued">
                              Saved segment presets for reuse when creating tests. Save targeting
                              as a preset in the test wizard.
                            </Text>
                          </div>
                        </div>
                        <div className={styles.panelCardBody}>
      ...
      <Modal
        ...
        secondaryActions={[
          { content: 'Cancel', onAction: () => setDeletePresetId(null) },
        ]}
      />
    </PageShell>
  );
