...
import { EyeFirstIcon, ExportIcon, LinkIcon, RefreshIcon, StarFilledIcon } from '@shopify/polaris-icons';
...
function Analytics() {
          ...
          <Card sectioned>
            <BlockStack gap="300">
              <Text as="p" variant="bodyMd">
                {errorMessage || 'No analytics data available.'} Check that the test has run and has visitor data.
              </Text>
            </BlockStack>
          </Card>
        </Page>
  ...
  return (
    <div className={`${pageShell.page} ${styles.analyticsPage}`} data-page="analytics">
      <Toast message={errorMessage || promoteError} type="error" onClose={() => setPromoteError(null)} duration={5000} />
      {successMessage && (
        <Toast
            ...
            <div className={styles.heroContent}>
              <div className={styles.heroLeft}>
                ...
                <div className={styles.heroMeta}>
                  ...
                  {testInfo?.started_at && testInfo?.stopped_at && (() => {
                    const days = Math.max(1, Math.round((new Date(testInfo.stopped_at) - new Date(testInfo.started_at)) / (24 * 60 * 60 * 1000)));
                    return <span className={styles.heroBadge} style={{ marginLeft: 8 }}>Duration: {days}d</span>;
                  })()}
                  {testInfo?.started_at && !testInfo?.stopped_at && testInfo?.status === 'running' && (() => {
                    const days = Math.max(0, Math.round((Date.now() - new Date(testInfo.started_at)) / (24 * 60 * 60 * 1000)));
                    return <span className={styles.heroBadge} style={{ marginLeft: 8 }}>Running: {days}d</span>;
                  })()}
                  {testInfo?.goal?.conversion_window_days && (
                    <span className={styles.heroBadge} style={{ marginLeft: 8 }}>Window: {testInfo.goal.conversion_window_days}d</span>
                  )}
                </div>
              </div>
              <div className={styles.heroRight}>
                <div className={styles.heroActions}>
                  ...
                  <div className={styles.heroSecondaryBar}>
                    ...
                    <button
                      ...
                      onClick={promoteCandidate ? () => { setPromoteError(null); setPromoteOpen(true); } : undefined}
                ...
                <div className={styles.heroQuickStats}>
                  ...
                  <div className={styles.heroStat}>
                    <span className={styles.heroStatValue}>{totalConversions.toLocaleString()}</span>
          ...
          {analytics.srm?.detected && (
            ...
            <Banner tone="warning" title="Sample ratio mismatch detected">
              <Text as="p" variant="bodySm">
                {analytics.srm.message || 'Traffic split deviates from expected allocation. This may indicate tracking issues, bot traffic, or assignment skew. Verify your implementation before trusting results.'}
              </Text>
            </Banner>
            </div>
          )}
        ...
        <Layout>
          ...
          <Layout.Section>
            <MetricGrid className={styles.statsGrid}>
              ...
              {winner && (() => {
                const control = variants.find(v => v.id !== winner.id && (v.name === 'Control' || v.name === 'control')) || variants.find(v => v.id !== winner.id) || variants[0];
                ...
                const testDays = testInfo?.started_at && testInfo?.stopped_at
                  ? Math.max(1, (new Date(testInfo.stopped_at) - new Date(testInfo.started_at)) / (24 * 60 * 60 * 1000))
                  : testInfo?.started_at
                    ? Math.max(1, (Date.now() - new Date(testInfo.started_at)) / (24 * 60 * 60 * 1000))
                    : 30;
                const monthlyVisitors = testDays > 0 ? (totalVisitors / testDays) * 30 : totalVisitors;
              ...
              <Card>
                <BlockStack gap="400">
                  <Text variant="headingLg" as="h2">
                    {analytics.significance.bayesian ? 'Bayesian Analysis' : 'Statistical Significance'}
                  </Text>

                  {analytics.significance.bayesian && analytics.significance.probToBeatControl ? (
                    <BlockStack gap="300">
                      <Text variant="bodySm" color="subdued" as="p">
                      ...
                      <div className={styles.bayesianGrid}>
                        {analytics.significance.probToBeatControl.map((item, idx) => (
                          <div key={item.variantId || idx} className={styles.bayesianCard}>
                            <Text variant="bodySm" fontWeight="semibold" as="p">{item.variantName || item.variantId}</Text>
                            <Text variant="headingMd" as="p">
                              {(item.probabilityToBeatControl * 100).toFixed(1)}%
                            </Text>
                            <Text variant="bodySm" color="subdued" as="p">
                              {item.probabilityToBeatControl >= 0.95 ? 'Strong evidence' : item.probabilityToBeatControl >= 0.8 ? 'Likely better' : 'Inconclusive'}
                            </Text>
                          </div>
                        ))}
                      </div>
                    ...
                    <Card sectioned>
                      <BlockStack gap="200">
                        ...
                        <Text variant="bodySm" as="p">
                          This variant performed best with a {analytics.significance.lift}% lift and{' '}
                          {(winner.conversionRate ?? 0).toFixed(2)}% conversion rate.
                        </Text>
                      </BlockStack>
                    </Card>
                  )}
              ...
              <Card>
                <BlockStack gap="400">
                  ...
                  <Text variant="bodySm" color="subdued" as="p">
                    ...
                    <code style={{ fontSize: '0.8em', background: 'var(--bg-tertiary)', padding: '2px 6px', borderRadius: 4 }}>
                    ...
                    <BlockStack key={eventName} gap="200">
                      ...
                      <div className="grid-responsive">
                        {variants.map((variant) => {
                          const ev = variant.secondaryEvents?.[eventName] || { count: 0, sum: 0, rate: 0 };
                          return (
                            <Card key={variant.id} sectioned>
                              <BlockStack gap="100">
                                ...
                                <Text variant="bodySm" color="subdued" as="p">
                                  {(variant.visitors ?? 0) > 0 ? `${(ev.rate ?? 0).toFixed(2)}% rate` : '—'}
                                </Text>
                              </BlockStack>
                            </Card>
                          );
                  ...
                  <div className="grid-responsive">
                    <Card sectioned>
                      <BlockStack gap="200">
                        ...
                        <Text variant="heading2xl" as="h2" fontWeight="bold">
                          $
                          {(analytics.revenueImpact.controlRevenue ?? 0).toLocaleString(undefined, {
                    ...
                    <Card sectioned>
                      <BlockStack gap="200">
                        ...
                        <Text variant="heading2xl" as="h2" fontWeight="bold">
                          $
                          {(analytics.revenueImpact.testRevenue ?? 0).toLocaleString(undefined, {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2,
                          })}
                        </Text>
                    ...
                    <Card sectioned>
                      <BlockStack gap="200">
                        ...
                        <Text
                          ...
                          as="h2"
                          fontWeight="bold"
                          color={(analytics.revenueImpact.impact ?? 0) > 0 ? 'success' : 'critical'}
                        >
      ...
      <Modal
        ...
        onClose={() => { setPromoteError(null); setPromoteOpen(false); }}
        ...
        secondaryActions={[
          {
            content: 'Cancel',
            onAction: () => { setPromoteError(null); setPromoteOpen(false); },
          },
        ]}
      >
        <Modal.Section>
