...
import { Page, Layout, Modal, Text, Icon } from '@shopify/polaris';
import {
  ChartLineIcon,
  DeleteIcon,
  DuplicateIcon,
...
function TestDetail() {
  ...
  const { data: test, isLoading: loading, isError, error } = useTest(id, {
  ...
  useEffect(() => {
    ...
    const isFromListWithVariants = toCache === listTest && Array.isArray(toCache.variants) && toCache.variants.length > 0;
  ...
  const handleStop = async (action) => {
    ...
    try {
      ...
      setErrorMessage(err.response?.data?.details?.[0] || err.response?.data?.error || 'Failed to stop test');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleRolloutSubmit = async () => {
    ...
    try {
      ...
      setSuccessMessage(`Rollout started at ${initialPercent}%. Will reach 100% in ${rolloutDuration} days.`);
    } catch (err) {
      setErrorMessage(err.response?.data?.details?.[0] || err.response?.data?.error || 'Failed to start rollout');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handlePersonalize = async () => {
    ...
    try {
      ...
      setErrorMessage(err.response?.data?.details?.[0] || err.response?.data?.error || 'Failed to apply winner');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleDisablePersonalization = async () => {
    ...
    try {
      ...
      setErrorMessage(err.response?.data?.details?.[0] || err.response?.data?.error || 'Failed to disable');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleSaveCode = async codePayload => {
    ...
    const updatedTest = response?.data?.test ?? response?.data?.data?.test;
    if (updatedTest) {
            ...
            <div className={styles.detailHeroGrid}>
              <div className={styles.detailHeroContent}>
                ...
                <div className={styles.detailHeroMeta}>
                  <span className={styles.detailHeroMetaItem}>{testTypeLabel}</span>
                  <span className={styles.detailHeroMetaDot} aria-hidden>·</span>
                  <span
                    className={`${styles.detailStatusPill} ${
                      test.status === 'running' ? styles.detailStatusRunning :
                      test.status === 'draft' ? styles.detailStatusDraft :
                      styles.detailStatusStopped
                    }`}
                  >
                    {test.status === 'running' ? 'Running' : test.status === 'draft' ? 'Draft' : 'Stopped'}
                  </span>
                  {test.variants?.length > 0 && (
                    <>
                      <span className={styles.detailHeroMetaDot} aria-hidden>·</span>
                      <span className={styles.detailHeroMetaItem}>{getVariantCount(test)} variants</span>
                    </>
                  )}
                </div>
              </div>
                  ...
                  <div className={styles.detailPersonalizationBadge}>
                    {isPersonalized ? (
                      ...
                      <span className={styles.badgeRollout}>
                        <Icon source={ChartVerticalFilledIcon} /> Rollout {test?.effective_rollout_percent ?? test?.rollout_percent ?? 0}%
                      </span>
                    )}
                  </div>
                  <button
              ...
              <div className={styles.detailActionsDivider} aria-hidden="true" />
              ...
              <div className={styles.rolloutInline}>
                ...
                <div className={styles.rolloutInlineBody}>
                  ...
                  <div className={styles.rolloutInlinePreview}>
                    <span>{rolloutInitialPercent}% → 100% in {rolloutDuration} days</span>
                    <div className={styles.rolloutInlineChart}>
                      <div
                        className={styles.rolloutInlineBar}
                        style={{
                          width: `${rolloutInitialPercent}%`,
                          background: 'linear-gradient(90deg, var(--futuristic-cyan), var(--futuristic-violet))',
                        }}
                      />
                    </div>
                  </div>
                  <div className={styles.rolloutInlineActions}>
                    ...
                    <button
                      ...
                      disabled={actionLoading}
                    >
                      {actionLoading ? <span className={styles.rolloutSubmitSpinner} /> : 'Start rollout'}
                    </button>
                  </div>
                </div>
              </div>
