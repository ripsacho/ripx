...
**Adding your server as the Shopify app (URLs, App Proxy, embedded app, script auto-install):** see [SHOPIFY_HOSTED_APP_SETUP.md](SHOPIFY_HOSTED_APP_SETUP.md).
...
- **Shopify OAuth:** `RIPX_OAUTH_REDIRECT_BASE` – **must match** Partner Dashboard **Application URL** exactly (e.g. from `shopify.app.toml` → `application_url`). Use a **stable** URL (e.g. custom domain); do **not** use a dynamic tunnel URL (e.g. `*.trycloudflare.com`) — it changes on restart and breaks OAuth. See [OAUTH_FIX.md](OAUTH_FIX.md). After deploy, call `GET /api/auth/oauth-redirect-uri` to verify; ensure `SHOPIFY_API_KEY` matches Partner Dashboard Client ID.
...
- **Shopify OAuth (if you use Shopify):**  
  Open `https://your-app-domain/api/auth/oauth-redirect-uri`. The JSON response includes `partnerDashboard.applicationUrl` and `partnerDashboard.allowedRedirectionUrl` — ensure these match **Application URL** and **Allowed redirection URL(s)** in Shopify Partner Dashboard. If you see `isDynamicTunnel: true` or get "redirect_uri and application url must have matching hosts", set `RIPX_OAUTH_REDIRECT_BASE` to a **stable** URL that matches Partner Dashboard and restart. See [OAUTH_FIX.md](OAUTH_FIX.md). The server logs the expected callback at startup when Shopify is configured.
...
| Invalid install link / OAuth host mismatch | See [OAUTH_FIX.md](OAUTH_FIX.md). Set `RIPX_OAUTH_REDIRECT_BASE` to the **same** stable URL as in Partner Dashboard (not a dynamic tunnel URL). Open `GET /api/auth/oauth-redirect-uri` and copy `partnerDashboard.applicationUrl` and `partnerDashboard.allowedRedirectionUrl` into Partner Dashboard. Ensure `SHOPIFY_API_KEY` in `.env` matches Partner Dashboard **Client ID** (and `client_id` in `shopify.app.toml`). |
