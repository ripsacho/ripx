...
**Verify:** After deployment, call `GET /api/auth/oauth-redirect-uri` (from the same host as your app). The response includes `partnerDashboard.applicationUrl` and `partnerDashboard.allowedRedirectionUrl` — copy these into Partner Dashboard. If `isDynamicTunnel: true`, switch to a stable URL and see OAUTH_FIX.md. The server also logs the expected callback URL at startup when Shopify is configured.
...
**Full guide:** See [OAUTH_ADD_STORE.md](OAUTH_ADD_STORE.md) for why this happens and how to avoid it.
