  ...
  const result = await query(
    ...
    const settings = row
      ? {
          ...
          autoStopEnabled: row.auto_stop_enabled !== false,
  ...
  await query(
      `INSERT INTO shop_settings (shop_domain, min_sample_size, confidence_level, auto_stop_enabled, 
       ...
       DO UPDATE SET
         ...
         updated_at = NOW()`,
      [shopDomain, minSample, confidence, !!autoStopEnabled, webhookUrl || null, JSON.stringify(events)]
    );
  ...
  res.json({
    ...
    integrations: {
      ...
      bigquery: {
        ...
        lastExportAt,
        hasCredentials: !!(dbConfig?.bigqueryCredentials || process.env.GOOGLE_APPLICATION_CREDENTIALS),
        hint: bqConfigured
 ...
 */
router.put('/integrations', asyncHandler(async (req, res) => {
  const shopDomain = req.shopDomain;
  if (!shopDomain) {
    return sendError(res, 401, 'Shop domain required');
  }
  ...
  const config = {};
  if (ga4MeasurementId !== undefined) {config.ga4MeasurementId = ga4MeasurementId;}
  if (ga4ApiSecret !== undefined) {config.ga4ApiSecret = ga4ApiSecret;}
  if (bigqueryProjectId !== undefined) {config.bigqueryProjectId = bigqueryProjectId;}
  if (bigqueryDataset !== undefined) {config.bigqueryDataset = bigqueryDataset;}
  if (bigqueryCredentials !== undefined) {config.bigqueryCredentials = bigqueryCredentials;}

  const dbConfig = await integrationConfig.getIntegrationConfig(shopDomain);
  const PLACEHOLDER = '••••••••';
  const mergeSecret = (val, existing) => {
    if (val === undefined) {return existing || '';}
    if (val === '' || val === PLACEHOLDER) {return existing || '';}
    return val;
  };
  const merged = {
    ga4MeasurementId: (config.ga4MeasurementId !== undefined ? config.ga4MeasurementId : dbConfig?.ga4MeasurementId) || '',
    ga4ApiSecret: mergeSecret(config.ga4ApiSecret, dbConfig?.ga4ApiSecret),
    bigqueryProjectId: (config.bigqueryProjectId !== undefined ? config.bigqueryProjectId : dbConfig?.bigqueryProjectId) || '',
    bigqueryDataset: (config.bigqueryDataset !== undefined ? config.bigqueryDataset : dbConfig?.bigqueryDataset) || 'ripx_analytics',
    bigqueryCredentials: (config.bigqueryCredentials === '[configured]' || config.bigqueryCredentials === PLACEHOLDER)
      ? (dbConfig?.bigqueryCredentials || '')
      : (config.bigqueryCredentials !== undefined ? config.bigqueryCredentials : dbConfig?.bigqueryCredentials) || '',
  };
