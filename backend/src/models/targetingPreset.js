...
async function createPreset(shopDomain, name, segments, goal = null, variants = null) {
  ...
  const variantsJson = variants && Array.isArray(variants) && variants.length > 0 ? JSON.stringify(variants) : null;
  const sql = `
    ...
    RETURNING *
  `;
  const result = await query(sql, [shopDomain, name, JSON.stringify(normalized), goalJson, variantsJson]);
