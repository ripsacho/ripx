// extensions/ripx-payment-customization/node_modules/@shopify/shopify_function/run.ts
...
// extensions/ripx-payment-customization/src/run.js
...
function buildHideOperations(paymentMethods = [], wantedNames = []) {
  return paymentMethods.filter((method) => matchesMethodName(method?.name, wantedNames)).map((method) => ({
    paymentMethodHide: {
...
function buildMoveOperations(paymentMethods = [], wantedNames = []) {
  const byName = new Map(
    paymentMethods.map((method) => [normalizeText(method?.name), method]).filter(([, method]) => method?.id)
  );
...
function cartPaymentMethodsTransformRun(input) {
  ...
  const operations = action === "rename" ? buildRenameOperations(paymentMethods, methodNames, matchedRule?.rename_to) : action === "reorder" ? buildMoveOperations(paymentMethods, methodNames) : buildHideOperations(paymentMethods, methodNames);
